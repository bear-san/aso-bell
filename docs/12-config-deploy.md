...
provider-slack   [serve|healthcheck|version] [--manager-addr ADDR] [--slack-bot-token ...] ...
provider-discord [serve|healthcheck|version] [--manager-addr ADDR] [--discord-bot-token ...] ...
# サブコマンドを省略すると serve。healthcheck は自プロセスの gRPC Health を叩く
```
